...
final class RouterTest extends TestCase
{
	public function testDirectoryRoutes()
	{
		...
		for($i = 0; $i < count($this->dir_external_scan); $i++) {
			if(
		...
		$this->router_external_scan = array();
		...
		foreach ($this->router_external_scan as $name) {
			$in_array = in_array($name, $this->dir_external_scan);
	...
	public function testRouterRules()
	{
		...
		foreach ($this->dir_external_scan as $key => $val) {
			if(
				$val == '.' ||
				$val == '..' ||
				$val == 'index.php'
		...
		foreach ($this->dir_external_scan as $file) {
			$in_array = in_array($file, $this->router_external_scan);

			if(!$in_array) {
				echo "\nMISSING  ".$file." from htaccess external API router";
			}
